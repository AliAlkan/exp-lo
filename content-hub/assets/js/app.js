document.addEventListener('click', (event) => {
  document.querySelectorAll('.create-menu[open], .doc-more-menu[open], .row-more-menu[open], .workspace-more-menu[open]').forEach((menu) => {
...
if (previewRoot) {
  ...
  const closeOptionMenus = () => {
    document.querySelectorAll('.doc-more-menu[open]').forEach((menu) => {
      menu.removeAttribute('open');
    });
  };
  const setSideDrawer = (activeDrawer) => {
    [drawer, detailsDrawer].forEach((currentDrawer) => {
      if (!currentDrawer) {
        return;
      }
